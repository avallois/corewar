...
/*   Updated: 2018/09/20 05:26:57 by plamusse         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		ssize_t ret)
{
	...
	*line = ft_strsub(lst_tmp->content, 0, i);
	if (!(tmp = ft_strsub(lst_tmp->content,
					i + 1,
					ft_strlen(lst_tmp->content) - i)))
		return (-1);
	free(lst_tmp->content);
	lst_tmp->content = tmp;
	return (1);
...
ssize_t				ft_addbuff(int const fd, t_list *lst_tmp, ssize_t ret)
{
	...
	if ((ret = read(fd, buff, GNL_BUFF_SIZE)) > 0)
	{
		buff[ret] = '\0';
		if (!(tmp = ft_strjoin(lst_tmp->content, buff)))
			return (-1);
		free(lst_tmp->content);
		lst_tmp->content = tmp;
		ret = 1;
...
int					get_next_line(int const fd, char **line)
{
	...
	if (fd < 0 || !line)
		return (-1);
	if (!(lst_tmp = ft_lstupdate(fd, stock, &stock)))
		return (-1);
	ret = 1;
	while (ret >= 0)
	{
